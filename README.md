...
| `frappe_list` | List records, with optional filters, ordering, pagination, and field selection. By default it returns at most ten fields total: the exact record `name` plus up to nine metadata-selected preview fields. It preserves the current title/list/search metadata priorities and uses the five additional slots to prioritize Currency fields, their companion currency fields, and custom fields. If `fields` is supplied, it returns `name` plus exactly those fields. The chat response is a compact record index containing count and exact record names; selected records and columns are returned for the canvas. |
