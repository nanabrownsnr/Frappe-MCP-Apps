...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "For ordinary lists, omit fields: frappe_list discovers a compact ten-field preview "
        ...
        "frappe_schema and pass the needed field names explicitly; the result then contains `name` "
        "plus exactly those fields. Do not repeat a list call just to obtain fields. If the user "
        "asks to open a listed record, call frappe_get with the "
        ...
        "the user explicitly asks to clear a field. To add rows to a child table, pass "
        "child_table_changes with operation 'append'; never replace the full child table. Use "
        "the exact child-table field and row field names from the parent and child DocType schemas. "
        "Do not claim a change succeeded unless the "
