...
function DealPipeline({ records }) {
    ...
    const total = records.reduce((sum, row) => sum + Number(row.deal_value ?? row.expected_deal_value ?? row.annual_revenue ?? 0), 0);
    return <section className="pipeline-view">
        ...
        <div className="pipeline-columns">{statuses.map((status) => <div className="pipeline-column" key={status}><h2>{label(status)} <small>{filtered.filter((row) => (row.status || "Uncategorized") === status).length}</small></h2>{filtered.filter((row) => (row.status || "Uncategorized") === status).map((deal, index) => { const value = deal.deal_value ?? deal.expected_deal_value ?? deal.annual_revenue; return <article className="deal-card" key={deal.name ?? index}><strong>{deal.title || deal.deal_name || deal.name}</strong><span>{deal.organization || deal.company || "No organization"}</span><b>{value ? `${deal.currency || ""} ${Number(value).toLocaleString()}` : "No value"}</b><span>{deal.expected_closure_date ? `Close ${deal.expected_closure_date}` : deal.deal_owner || deal.owner ? `Owner: ${deal.deal_owner || deal.owner}` : ""}</span>{serviceKey && deal[serviceKey] ? <em>{deal[serviceKey]}</em> : null}</article>})}</div>)}</div>
    </section>;
}
